...
for train_index, test_index in kf.split(X):
    ...
    err = y_test-y_pred
    fname = input("Input Plot Filename here: ")
    fig = plt.figure()
    plt.scatter(test_index,y_pred,label="Predicted Data")
    plt.scatter(test_index,y_test,label="Actual Data")
    plt.legend()
    plt.xlabel("Data Point ID")
    plt.ylabel("Particle Volume (mm^3)")
    plt.title("Predicted vs. Actual Comparison using Gaussian Process Regression")
    plt.savefig(fname)
